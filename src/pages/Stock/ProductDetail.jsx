      ...
      <div className="flex-1 ml-64 min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center gap-4">
                <span className="px-4 py-2 bg-green-100 text-green-800 rounded-lg font-bold text-lg border-2 border-green-300">
                  📦 Available: {formatQty(available.boxes, available.pieces)}
                </span>
                {product.description && (
                  <span className="text-gray-600 text-sm max-w-md truncate">
                    {product.description}
                  </span>
                )}
                {product.lowStockThreshold > 0 && (
                  <span className="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-lg text-sm font-semibold border border-yellow-300">
                    ⚠️ Low Stock Alert: {product.lowStockThreshold} boxes
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="space-y-4">
                {/* Inventory Breakdown - Compact */}
                <div className="pb-4 border-b border-gray-200">
                  <p className="text-sm font-semibold text-gray-700 mb-3">Inventory Breakdown</p>
                  <div className="grid grid-cols-4 gap-3">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">📦</span>
                      <div>
                        <p className="text-xs text-gray-600">Stock</p>
                        <p className="text-sm font-bold text-blue-600">{formatQty(product.stock?.boxes || 0, product.stock?.pieces || 0)}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-lg">💰</span>
                      <div>
                        ...
                        <p className="text-sm font-bold text-green-600">{formatQty(product.sales?.boxes || 0, product.sales?.pieces || 0)}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-lg">⚠️</span>
                      <div>
                        <p className="text-xs text-gray-600">Damage</p>
                        <p className="text-sm font-bold text-red-600">{formatQty(product.damage?.boxes || 0, product.damage?.pieces || 0)}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-lg">🔄</span>
                      <div>
                        ...
                        <p className="text-sm font-bold text-yellow-600">{formatQty(product.returns?.boxes || 0, product.returns?.pieces || 0)}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-sm text-gray-600 mb-1">Pieces Per Box</p>
          ...
          <div className="flex gap-4 justify-center mb-8">
            <button
              onClick={() => navigate(`/stock/edit-product/${productId}`)}
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-8 max-w-md mx-4">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-2xl font-bold text-gray-900">⚠️ Confirm Delete</h3>
                  <button
